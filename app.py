...
# ==============================================================================
# MÓDULO 3: VIGAS COMBINADAS, GRÁFICO REAL Y MÓDULO RESISTENTE (STEINER)
# ==============================================================================
    ...
    st.markdown("Visualización geométrica detallada de perfiles **IPN** y **UPN** soldados con ubicación baricéntrica exacta.")

    # BD de Perfiles Estándar (cm, cm2, cm4)
    cat_ipn = {
    ...
    with col_ctrl:
        ...
        if agregar_p2:
            ...
            preset = st.radio(
                ...
                [
                    "Ala Superior (Viga Carril - UPN Acostado alas hacia abajo)",
            ...
            # Posiciones predeterminadas inteligentes
            if preset == "Ala Superior (Viga Carril - UPN Acostado alas hacia abajo)":
                rot2 = 270 if tipo_p2 == "UPN" else 90
                def_x = 0.0
                def_y = p1["h"] + (p2["ey"] if tipo_p2 == "UPN" else p2["b"]/2.0)
                ...
                rot2 = 0
                def_x = (p1["b"]/2.0) + (p2["ey"] if tipo_p2 == "UPN" else p2["b"]/2.0)
                def_y = p1["h"]/2.0
            else:
                rot2 = 0
            ...
            with col_dx:
                dx2 = st.number_input("Coordenada X Baricentro 2 (cm):", value=float(def_x), step=0.5)
            with col_dy:
                dy2 = st.number_input("Coordenada Y Baricentro 2 (cm):", value=float(def_y), step=0.5)

            rot_p2 = st.selectbox("Orientación Perfil 2 (Grados):", [0, 90, 180, 270], index=[0, 90, 180, 270].index(rot2))
            ...
            if rot_p2 in [90, 270]:
                ...
                h2_efec, b2_efec = p2["b"], p2["h"]
            else:
                ix2_local, iy2_local = p2["ix"], p2["iy"]
                h2_efec, b2_efec = p2["h"], p2["b"]
        else:
    ...
    # ==========================================================================
    # FUNCIÓN PARA GENERAR POLÍGONOS REALES DE IPN Y UPN
    # ==========================================================================
    def obtener_poligono_perfil(p_type, p_data, x_center, y_center, rot_deg):
        ...
        # Geometría base centrada en el baricentro del perfil (0,0)
        if p_type == "IPN":
            verts = [
        ...
        else: # UPN (Apertura nativa hacia la derecha +X, baricentro en (0,0))
            x_back = -ey
            x_web_in = -ey + tw
            x_tip = b - ey
            verts = [
        ...
        # Aplicar rotación sobre el baricentro
        rad = math.radians(rot_deg)
        cos_r, sin_r = math.cos(rad), math.sin(rad)
        
        verts_rot = []
        for vx, vy in verts:
            rx = vx * cos_r - vy * sin_r + x_center
            ry = vx * sin_r + vy * cos_r + y_center
            verts_rot.append((rx, ry))
    ...
    # ==========================================================================
    with col_vis:
        st.subheader("🖼️ Representación Real de la Sección Compuesta")

        fig, ax = plt.subplots(figsize=(6, 6))

        # Dibujar Perfil 1 (Base)
        ...
        # Marcar Ejes Neutros y Baricentro Compuesto
        ...
        # Determinar límites visuales dinámicos
        ...
        ax.set_xlabel("X [cm]")
        ax.set_ylabel("Y [cm]")
        ax.set_title("Corte Transversal Real de la Viga", fontsize=11, fontweight='bold')
        ax.grid(True, linestyle='--', alpha=0.5)
        ax.legend(loc='upper right', fontsize=8)

        st.pyplot(fig)

    # Cálculo final de Módulos Resistentes
